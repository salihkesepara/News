...
//      console.log(result);
  ...
  self.generateValue = function (data) {
    var valueQuery = [];
    if (angular.isArray(data)) {
      for (var i = 0; i < data.length; i++) {
        valueQuery.push("" + data[i] + "");
      }
    } else {
      valueQuery.push('' + data + '');
    }
    return valueQuery;
  };
  
  self.generateWhereQuery = function (field) {
    var WhereQuery = [];
    if (angular.isArray(field)) {
      for (var i = 0; i < field.length; i++) {
        WhereQuery.push("" + field[i] + " = ?");
      }
    } else {
      WhereQuery.push('' + field + ' = ?');
    }
    return WhereQuery;
  };
